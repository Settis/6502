...
: COMPILE ( -- )
    ...
    2+    \ Add 1 befause 6502 save it such a way and another 1 to jump over JSR
    DUP 1+ >R \ Increment this pointer by another 1 (3 in total) to point to the second word following
...
: BEGIN ( -- addr n ) \ at compile time
    ?COMP \ Issue an error message if not compiling.
    HERE  \ Push dictionary pointer on stack 
    1     \ Error checking number.
; IMMEDIATE

: BACK ( addr -- )
    , \ Compile the backward branching address.
;
...
: (.") ( -- )
    R       \ Copy EP from the return stack, which points to the beginning of the in-
            \ line text string.
    1+   \ Add for shift JSR return addr
...
: INTERPRET ( -- )
    BEGIN                   \ Start the interpretation loop
        ...
        IF                  \ A matching entry is found. Do the following:
            ...
            IF CFA 
                20 C,   \ compile JSR
            ,        \ Compile the address of the function
